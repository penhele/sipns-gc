...
export default function TeacherPage() {
  ...
  const { data: scoresData, isLoading: isLoadingScores } = useScores(me?.teacher?.id);
  ...
  const averageScore = totalAssessed > 0 
    ? (scores.reduce((acc, curr) => acc + curr.nilaiAkhir, 0) / totalAssessed).toFixed(1)
    : "0.0";
    ...
    <div className="flex flex-col space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="mt-2 text-indigo-100 max-w-xl">
              Kelola nilai akademik siswa Anda, pantau statistik kelulusan kelas, dan perbarui hasil ujian di sini.
            </p>
          </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-muted-foreground text-sm font-medium">Memuat data dashboard...</p>
        </div>
      ) : (
        <>
          {/* Key Metrics */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="bg-gradient-to-br from-indigo-500/10 to-indigo-600/5 border-indigo-200/50 dark:border-indigo-800/50 hover:shadow-md transition-all duration-300">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-sm font-medium text-indigo-700 dark:text-indigo-400">Total Dinilai</CardTitle>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">Siswa telah menerima penilaian</p>
              </CardContent>
            </Card>
            ...
            <Card className="bg-gradient-to-br from-emerald-500/10 to-emerald-600/5 border-emerald-200/50 dark:border-emerald-800/50 hover:shadow-md transition-all duration-300">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-sm font-medium text-emerald-700 dark:text-emerald-400">Lulus</CardTitle>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">Siswa memenuhi nilai kelulusan (&ge;75)</p>
              </CardContent>
            </Card>
            ...
            <Card className="bg-gradient-to-br from-purple-500/10 to-purple-600/5 border-purple-200/50 dark:border-purple-800/50 hover:shadow-md transition-all duration-300">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-sm font-medium text-purple-700 dark:text-purple-400">Rata-rata Nilai</CardTitle>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">Akumulasi nilai akhir kelas</p>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <Link href={ROUTES.NILAI}>
                <Card className="hover:border-primary/50 cursor-pointer transition-all duration-300 group hover:-translate-y-0.5">
                  <CardContent className="p-6 flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-bold text-foreground">Kelola Semua Nilai</h3>
                      <p className="text-xs text-muted-foreground">Lihat, sunting, dan hapus nilai siswa.</p>
                    </div>
                  </CardContent>
                </Card>
              </Link>
              ...
              <Link href={ROUTES.REKAP}>
                <Card className="hover:border-primary/50 cursor-pointer transition-all duration-300 group hover:-translate-y-0.5">
                  <CardContent className="p-6 flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-bold text-foreground">Rekap Nilai Siswa</h3>
                      <p className="text-xs text-muted-foreground">Analisis statistik kelulusan & rata-rata.</p>
                    </div>
                  </CardContent>
                </Card>
              </Link>
            ...
            <div className="flex flex-col space-y-4 lg:col-span-2">
              <h2 className="text-lg font-bold text-foreground">Penilaian Terbaru</h2>
                          ...
                          {scores.slice(0, 5).map((score) => (
                            <tr key={score.id} className="hover:bg-muted/10 transition-colors">
                              <td className="px-6 py-4">
                                ...
                                <div className="text-xs text-muted-foreground">{score.student?.class}</div>
                              </td>
